...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let filtArr = getBaseStatGreaterThan(arr,minBaseStat);

  return filtArr.map(obj => obj.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  return arr.filter(obj => !obj.hasOwnProperty('children'));
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  let numOnly = arr.filter(itm => typeof(itm) === 'number');

  return numOnly.map(num => {
    if (num % 2 === 0){
      return 'even';
    } else {
      return 'odd';
