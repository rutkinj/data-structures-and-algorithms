...
const screenForNames = (arr) => {
  // Solution code here...
  let r = /(^(?:Mr|Mrs|Ms|Dr).\s[a-zA-Z]+$)/;
  return arr.filter((name) => r.test(name));
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let heftyArr = arr.filter(char => char.mass > 77);
  let heftyNames =  heftyArr.map(char => char.name);
  return heftyNames.join(' - ');
  //this is ripe for reduce but I am not ripe for reduce
};
