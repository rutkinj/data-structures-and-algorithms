...
function transformToLis(obj){
  // Solution code here...
  return Object.keys(obj).map(k => `<li>${k}: ${obj[k]}</li>`);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(arr => arr.filter(item => typeof item === 'number' && item%5 === 0).map(num => Math.pow(2,num)));
  // lol what a mess
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(obj => obj.gender === 'male' || obj.gender === 'female').map(gendereds => gendereds.name).join(' and ');
};
